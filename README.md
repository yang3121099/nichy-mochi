...
```text
Mochi 收到了：command.sh · 版本 a31c29e4
Mochi 正在运行：command.sh · 版本 a31c29e4
训练输出……
Mochi 完成了 ✓：command.sh · 版本 a31c29e4
```
...
版本号对应保存下来的启动指令。其引用的外部代码和数据在执行时读取；排队期间请保留对应文件。
...
`tail -F` 持续显示新输出；`cat -n` 只查看当前内容并显示行号。

| 文件 | 内容 |
|---|---|
| `log` | 接收回执、运行输出、完成或失败状态 |
| `status.txt` | 当前任务、最新版本和更新时间 |
| `receipt.json` | 最新任务的完整版本与 GPU 读取回执 |
