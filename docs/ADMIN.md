...
Nichy 表示提交行为，Mochi 表示 GPU 执行端。事件日志固定采用北京时间（UTC+8），显示完整日期和时间，省略偏移后缀；已有日志不重写，程序原始输出不改动。

显示编号采用提交时刻的北京时间自然小时，如 `2026-09-18 14时 · 第 1 次提交`，每小时从 1 开始。编号在输出和复制备份前原子保存到 `.submissions.json`；重启后保持，归档后不复用，并发提交各有编号。`receipt.json` 的 `submission_label` 为显示文本，`submission_hour` 为日期与小时，`hour_submission` 为小时内序号，`backup` 为相对接头点的备份路径。为兼容旧版，`submission` 保留全局递增编号，`revision` 保留完整 SHA-256。不要删除或编辑 `.submissions.json`。

`backups/command_日期_小时h_序号.sh` 复制自每次提交已保存的指令快照，并校验内容；重复提交也有独立文件。备份不覆盖、不自动删除，不包含指令引用的业务代码和数据。清理任务目录不影响备份。升级时，会为仍有可验证快照的旧 `command.sh` 补建备份；已经删除的快照无法补回。

旧任务升级后也能查询编号；此前已有日志会保留原格式。当前队列还未被 Mochi 登记的提交显示“等待 Mochi 编号”。

## 日历与彩蛋

彩蛋后缀按提交时刻计算并保存，后续排队、执行和完成时沿用。周五全天、官方节假日全天、工作日 22:30 后分别显示问候；调休上班日按工作日处理，普通周末不当作节假日。

定时问候与 GPU worker 一同运行，最多每分钟计算一次，不请求网络或 GPU。每天 22:30、周五 09:00 / 12:00 / 17:00、节假日 09:00 写入 `log`，仅在预定时刻起 10 分钟内补发。`.reminders.json` 记录已发送事件，重启不重复；极端中断可能漏掉一条问候，不会重跑任务。

`holidays.json` 的 `years` 按年份映射，每年包含 `holidays`（日期到节日名称的映射）、`workdays`（调休上班日期数组）和可选 `source`（官方来源链接）。首次生成内置 2026 年数据，后续启动保留已有文件。新增年份应按官方公布安排补充，修改后重启读取。缺失年份或格式错误会在日志提示，暂停无法确认的节假日及工作日后缀；周五和每日定时问候继续。日历问题不停止任务执行。
