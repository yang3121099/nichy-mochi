...
Nichy 表示提交行为，Mochi 表示 GPU 执行端。事件日志包含完整日期、时间和时区偏移；已有日志不重写，程序原始输出不改动。

Mochi 按登记顺序为每个任务分配独立编号，并在输出前原子保存到 `.submissions.json`。重启沿用原编号，已归档任务的编号不复用；并发提交也使用不同编号。`submissions.tsv` 是便于查找的索引，`receipt.json` 的 `submission` 字段是最新编号，`revision` 仍保留原来的完整 SHA-256。编号代表一次提交，校验值代表指令内容，两者用途不同。不要删除或编辑 `.submissions.json`。
